...
def jacobi_mpi_cart_optimized(N, nx, ny, max_iter=1000000, tol=1e-9, L=1.0, block_size=50):
    ...
    # --- 1. Configuração da Grade e Abort ---
    if rank == 0:
        try:
    ...
    # --- 2. Lógica Cartesiana (Bypass para size == 1) ---
    if size == 1:
        # Modo sequencial otimizado (sem cart e sem buffers de comunicação)
        cart = None
        coords = [0, 0]
    ...
    # Aplicar as condições de contorno de Dirichlet nulas nas bordas globais (se for um processo de borda)
    # Note: Em Jacobi 2D, a inicialização com zeros já trata as bordas Dirichlet de U e Unew.
