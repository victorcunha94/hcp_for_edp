...
def jacobi_mpi_cart(N, nx, ny, max_iter=10000, tol=1e-8, L=1.0, block_size=5):
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()
...
def main():
    ...
    meta, comm_log = jacobi_mpi_cart(args.N,args.nx, args.ny,  max_iter=args.max_iter, tol=args.tol, local_iters=args.local_iters )
    ...
    if rank == 0:
        ...
        df.to_csv(f"results_{nx}x{ny}.csv", index=False)
        print(f"[rank 0] Arquivo results_{nx}x{ny}.csv salvo.")
