...
def jacobi_mpi_cart_optimized(N, nx, ny, max_iter=1000000, tol=1e-10, L=1.0, block_size=50):
    ...
    f_local[1:local_nx+1, 1:local_ny+1] = np.sin(2*np.pi * x) * np.sin(2*np.pi * y)
...
def main():
    ...
    # Coletar dados de TODOS os processos no rank 0
    ...
    if rank == 0:
        ...
        # Filtrar apenas processos válidos (sem erro)
        valid_metas = []
        for m in all_meta:
        ...
        if valid_metas:
            ...
            # Debug: verificar se U_data está presente
            if 'U_data' in df.columns:
                total_points = sum(len(row['U_data']) if isinstance(row['U_data'], list) else 0 
                                 for _, row in df.iterrows())
                print(f"Total de pontos U_data: {total_points}")
            else:
                print("AVISO: Coluna 'U_data' não encontrada no CSV")
        else:
