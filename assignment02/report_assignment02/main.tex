...
\title{
    ...
    \textbf{\Huge Regiões de Estabilidade e Computação Paralela} \\
...
Para validação do código utilizamos a região de estabilidade do método de Euler implícito do Livro \cite{leveque2007finite}

\begin{figure}[H]
\centering
\subfigure[Euler Implícito pelo código.  \label{fig:euler_python}]{
\includegraphics[width=.38\textwidth]{Figuras/PYTHON/stability_basic_1threads.png}
} % end subfigure
\quad % dá um espaço entre as duas figuras.
\subfigure[Euler Implícito na literatura. \label{fig:euler_leveque}]{
...
Nesta sessão discutiremos a implementação do código em paralelo e a estratégia adotada. Utilizamos o compilador \textit{\href{https://numba.pydata.org/}{Numba}} e a estratégia por "blocos".
...
\subsection{Numba}
O \textbf{Numba} é um compilador \textit{just-in-time} (JIT) para Python que acelera significativamente funções que utilizam operações numéricas, loops e bibliotecas como NumPy através de decoradores. \cite{lam2015numba}
...
Podemos verificar através do gráfico que houve ganho de performance com o aumento dos threads, porém, próximo do número de núcleos físicos, que são 8, o speedup não aumenta; um indício de de que o hyper-threading não ajuda no problema em que estamos resolvendo. 
...
onde $R(z)$ é o \textbf{polinômio de estabilidade}, cujas raízes determinam a região de estabilidade no plano complexo. Um ponto $z$ é considerado estável se $|R(z)| < 1$ após $T$ iterações.
 ...
 Este esquema combina a eficiência computacional do método explícito (preditor) com a estabilidade superior do método implícito (corretor), sendo particularmente adequado para problemas de estabilidade numérica onde se requer alto grau de precisão. Podemos verificar abaixo as regiões de estabilidade dos Métodos \textit{Adamns Basforth Moulton}, os mesmos resultados podem ser vistos em \cite{math10122015}. 
