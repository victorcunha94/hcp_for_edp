...
def ABM3_step(u_n, u_nm1, u_nm2, z, n_corr=1):
    """
    Adams-Bashforth-Moulton de ordem 3 (ABM3)

    Convenção:
    u_n = U^n (mais recente), u_nm1 = U^{n-1}, u_nm2 = U^{n-2}

    Esquema P(EC)ᴺ:
    Preditor: Adams-Bashforth ordem 3
    Corretor: Adams-Moulton ordem 3

    Parameters:
    u_n: array [real, imag] - solução no tempo n
    u_nm1: array [real, imag] - solução no tempo n-1
    u_nm2: array [real, imag] - solução no tempo n-2
    z: array [real, imag] - parâmetro complexo
    n_corr: int - número de iterações corretor

    Returns:
    Ypre: array - preditor (Adams-Bashforth ordem 3)
    Ycor: array - corretor (Adams-Moulton ordem 3)
    """
    # Calcula f(u) para os pontos históricos
    f_n = complex_prod(z, u_n)  # f(u_n)
    f_nm1 = complex_prod(z, u_nm1)  # f(u_{n-1})
    f_nm2 = complex_prod(z, u_nm2)  # f(u_{n-2})

    # --- Preditor AB3: Ypre = u_n + (23/12)*f_n - (4/3)*f_{n-1} + (5/12)*f_{n-2} ---
    ...
    for _ in range(n_corr):
        # Adams-Moulton ordem 3: Ycor = u_n + (5/12)*f_est + (2/3)*f_n - (1/12)*f_{n-1}
...
def process_single_point_numba(ix, iy, xl, xr, yb, yt, incle, T, tol, n_corr):
    ...
    # --- ITERAÇÃO TEMPORAL ---
    for n_step in range(3, T):  # Começa do passo 2 (já temos u⁰ e u¹)
        Ypre, Ycor = ABM3_step(u2, u1, u0, z, n_corr)  # u1 = u_n, u0 = u_{n-1}

        # --- ATUALIZA HISTÓRICO ---
        u0 = u1  # u_{n-1} = u_n antigo
        u1 = u2
        u2 = Ycor  # u_n = novo valor
...
def process_all_points_compiled(xl_val, xr_val, yb_val, yt_val, incle_val, T_val, tol_val, num_threads):
    ...
    for idx in prange(total_points):
        ...
        real_z, img_z, is_stable, is_unstable = process_single_point_numba(
            h, k, xl_val, xr_val, yb_val, yt_val, incle_val, T_val, tol_val, 10
        )
