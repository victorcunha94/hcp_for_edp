...
@njit
def additional_compute_bound_work(z, iterations=5):
    result = z.copy()
    for i in range(iterations):
        result = complex_add(result, complex_scale(z, np.sin(i * 0.1)))
        result = complex_prod(result, complex_scale(z, np.cos(i * 0.1)))
    return result

#
# @njit
# def process_single_point_numba(h, k, xl, xr, yb, yt, incle, T, tol):
#     # real_z = xl + (h * abs(xr - xl) / incle)
#     # img_z = yb + (k * abs(yt - yb) / incle)
#     real_z = xl + (k * abs(xr - xl) / incle)
#     img_z = yb + (h * abs(yt - yb) / incle)
#
#     z = np.array([real_z, img_z])
#
#     z = additional_compute_bound_work(z, 5)  # Reduzido para teste
#
#     Un = np.array([1.0, 0.0])
...
#
#     #current = Un3.copy()
#     #for n in range(min(T, 100)):
#     for n in range(T):
#         Un1 = euler_implict_numba(Un, z)
#         Un = Un1
#
#         norm = complex_norm(Un)
#         if norm < tol:
#             is_stable = True
#             break
#         elif norm > 1.0 / tol:
...
@njit
def process_single_point_numba(h, k, xl, xr, yb, yt, incle, T, tol):
    # Coordenadas CORRETAS: h = linha (y), k = coluna (x)
    real_z = xl + (k * abs(xr - xl) / incle)  # x varia com k
    img_z = yb + (h * abs(yt - yb) / incle)  # y varia com h
    z = np.array([real_z, img_z])

    #z = additional_compute_bound_work(z, 2)


    u_current = np.array([1.0, 0.0])  # u₀ = 1 + 0i

    is_stable = False
    is_unstable = False


    for n in range(T):  # Limite de segurança
        u_next = euler_implict_numba(u_current, z)  # u_{n+1} = f(u_n)
        u_current = u_next  # Atualiza para próxima iteração

        norm = complex_norm(u_current)
        if norm < tol:
            is_stable = True
            break
        elif norm > 1.0 / tol:
...
def plot_stability_region_by_thread(real_coords, imag_coords, stable_flags, unstable_flags, thread_ids, num_threads,
    ...
    for i in range(len(real_coords)):
        if stable_flags[i]:
            ...
            plt.plot(real_coords[i], imag_coords[i], 'o', color=color, markersize=1, alpha=0.7)
        elif unstable_flags[i]:
            plt.plot(real_coords[i], imag_coords[i], 'ko', markersize=0.5, alpha=0.3)
...
if __name__ == "__main__":
    ...
    try:
        result = process_all_points_compiled(-4.0, 4.0, -2.5, 2.5, 5, 10, 1e-5, 1)
        ...
        for n_threads in threads_to_test:
            ...
            plot_stability_region_basic(
                ...
                f"output/stability_basic_{n_threads}threads.png",
                xl=-1.0, xr=3.0, yb=-2.0, yt=2.0
             )
            #
            #
            plot_stability_region_by_thread(
                ...
                n_threads,
                f"output/stability_by_thread_{n_threads}threads.png",
                xl=-1.0, xr=3.0, yb=-2.0, yt=2.0)
