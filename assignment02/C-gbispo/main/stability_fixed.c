...
static inline int is_stable_trbdf2_bruteforce(double complex z, double tolsup, double tolinf, int tid) {
    // Implement canonical TR-BDF2 ("half-step trapezoid then BDF2")
    // This uses the common formulation: y* = Trapezoid step with tau/2,
    // then y_{n+1} = (4*y* - y_n) / (3 - z) where z = lambda * tau.
    double complex u = 1.0;
    for (int i=0;i<3000;++i) {
        // trapezoid with half-step (tau/2):
        double complex R1 = (1.0 + z/4.0) / (1.0 - z/4.0);
        double complex u_star = R1 * u;
        // BDF2 stage (linear form): (1 - z/3) u_{n+1} = (4*u_star - u)/3
        double complex u_next = (4.0 * u_star - u) / (3.0 - z);
        u = u_next;
        double mag = cabs(u);
        if (mag > tolsup) return -1;
        if (mag < tolinf) return tid;
...
static inline int is_stable_pece_bruteforce(double complex z,
    ...
    for (int step = 0; step < 10000; ++step) {
        ...
        for (int k = 0; k < nCE; ++k) {
            double complex f_est = z*u_est; // <-- EVALUATION step! (evaluates f at current iterate)
        ...
        // -------------------------------
        // Final evaluation to complete PECE (make sure we evaluate f at last corrected value)
        // -------------------------------
        double complex f_final = z * u_est; (void)f_final; // computed but not used further; keeps the E in PECE
