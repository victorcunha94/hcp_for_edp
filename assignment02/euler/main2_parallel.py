...
if len(sys.argv) > 1:
    n_jobs = int(sys.argv[1])  # primeiro argumento: número de processos
if len(sys.argv) > 2:
    incle = int(sys.argv[2])   # segundo argumento: incle
...
results = Parallel(n_jobs=n_jobs, prefer="processes")(
    delayed(process_point)(h, k, tipo) for h in range(incle) for k in range(incle)
)
